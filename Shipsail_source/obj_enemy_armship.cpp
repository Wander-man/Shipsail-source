...
sf::Texture Armship::s_text{};

Armship::Armship(const crdvect &path, float speed, int hp, float htime, int type) 
...
bool Armship::checkcoll(prlist& prlist)
{
...
void Armship::manage(sf::RenderWindow &window, prlist& prlist, float currtime)
{
...
void Armship::settexture(const char *tfile)
{
...
void Armship::create(const crdvect& path, float speed, int hp, float htime, elist& elist)
{
    elist.push_back(std::make_shared<Armship>(path, speed, hp, 0.15f, 1));
}